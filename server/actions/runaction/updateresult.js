...
const updateResult = async (action, func, node_id, user_id, date, result, label) => {
  const existing = await get({table: 'results', columns: ['result_id'], conditions: {columns: {action, function: func, node_id, label, user_id}}})
  ...
  else await insert('results', {data: JSON.stringify(result), date, action, function: func, node_id, label, user_id})
}
