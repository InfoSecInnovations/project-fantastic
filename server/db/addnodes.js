...
const addNodes = async nodes => {
  ...
  for (const n of nodes) {
    ...
    else matches = await (n.ips && n.ips.length ? all({table: 'ips', columns: ['node_id'], conditions: {columns: {ip: n.ips}, compare: 'IN'}}) : Promise.resolve([])) // if we can't find the MAC Address, find nodes with the same IP address as this one
    ...
    if (matches.length == 1) await update({table: 'nodes', row: columns.reduce((result, v) => ({...result, [v]: n[v]}), {date}), conditions:{ columns: {node_id: matches[0].node_id}}}) // if there's just one node we update it
