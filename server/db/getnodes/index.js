...
const getNodes = async query => {
  ...
  for (const r of rows) {
    ...
    const connections = await GetConnections(db, ips, query)
    const valid_connections = connections.length || await db.get({table: 'connections', conditions: {groups: ConnectionConditions('to', ips, query)}})
