...
const addConnections = async (node_id, connections, is_remote) => {
  ...
  for (const c of connections) {
    const hostname = is_remote ? await get({table: 'nodes', columns: ['hostname'], conditions: {columns: {node_id}}}).then(res => res.hostname) : ''
    const name = processes[c.process] || await GetProcess(c.process, hostname)
    processes[c.process] = name
    let process_id
    const process = await get({table: 'processes', columns: ['process_id'], conditions: {columns: {pid: c.process, node_id}}}) // find the process in the relevant table
    if (process) {
      ...
      process_id = await insert('processes', {pid: c.process, name, node_id}) // if we didn't find a process, insert it
    }
