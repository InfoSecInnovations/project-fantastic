...
const runScan = async (db, scan, user, date, nodes, parameters, age, parent_event_id, parent_event_type = 'quest') => {
  ...
  const rows = await db.all({table: 'nodes', conditions: {groups: [{columns: {access: data.hosts}, compare: 'IN'}, {columns: {node_id: nodes}, compare: 'IN'}]}})
  const results = []
  for (const action of data.actions) {
    for (const row of rows) {
      ...
      if (data.pass === "review") results.push({node_id: row.node_id, result: result.results, filter: result.filter, action: action_path})
