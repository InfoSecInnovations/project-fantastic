export default (state, send) => {
  ...
  .then(res => res.forEach(v => {
    send({type: 'quest_results', quest: v.quest, date: v.date, results: JSON.parse(v.results), test_id: v.test_id, select: false})
  ...
  .then(res => res.json())
  .then(res => res.forEach(v => {
    send({type: 'test_results', test: v.test, parameters: JSON.parse(v.parameters), date: v.date, test_id: v.test_id, results: JSON.parse(v.results), select: false})
  ...
  .then(res => {
    res.result_rows.forEach(v => {
      ...
      if (story_node.type == 'tests') {
        send({type: 'story_results', story: v.story, node: v.story_node_id, date: v.date, test_id: v.test_id, results: JSON.parse(v.results), select: false})
